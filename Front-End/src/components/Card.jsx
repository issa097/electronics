...
function Card({ filterProductName, product, filterProduct, filterSubProducts, filterProductNames }) {
    return (
        <>
            <Search
                product={product}
            ...
            <div className="flex justify-center flex-wrap">
                {filterProductName.map((product, index) => (
                    <Link to={`/details/${product.product_id}`} key={index} className="max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 m-2" >
                        <a href="#">
